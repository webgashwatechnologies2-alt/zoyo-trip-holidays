...
export default function NationalDestinationClient({
  params,
}: {
  params: { slug: string };
}) {
  ...
  if (!dest && slug.includes('himachal')) {
    dest = {
      ...
      name: 'Himachal Pradesh Devbhoomi',
      type: 'national',
      region: 'North India',
      tagline: 'Snow-capped peaks, pine forests, adventure valleys & wooden temples.',
      description: 'Explore Devbhoomi Himachal with Manali, Shimla, Dharamshala, Dalhousie, and Spiti Valley.',
      heroImage: 'https://images.unsplash.com/photo-1626621341517-bbf3d9990a23?q=80&w=1600&auto=format&fit=crop',
      bestTime: 'All Year Round (Snow in Winter, Pleasant in Summer)',
      packagesCount: himachalPackages.length,
      packages: himachalPackages,
    };
  }

  // Generic fallback for any other national destination in list
  if (!dest) {
    const formattedName = slug.replace(/-/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase());
    dest = {
      id: `dest-${slug}`,
      slug: slug,
      name: `${formattedName} Tour Packages`,
      type: 'national',
      region: 'India',
      tagline: `Explore the vibrant culture, stunning landscapes, and curated holidays in ${formattedName}.`,
      description: `Experience handcrafted travel itineraries in ${formattedName} with dedicated private transport, audited hotel stays, and 24/7 on-ground assistance by Zoyo Trip Holidays.`,
      ...
      packages: [
        {
          id: `${slug}-1`,
          slug: `${slug}-classic-holiday`,
          title: `${formattedName} Classic Highlights Holiday`,
          subtitle: `Complete sightseeing, deluxe resort stays, private cab transfers & curated meals in ${formattedName}`,
          ...
          overview: `Discover the top attractions and cultural highlights of ${formattedName} on this premium, stress-free holiday package tailored for families, couples, and groups.`,
          highlights: [
            `Top iconic attractions and viewpoints of ${formattedName}`,
            'Comfortable private sanitized AC vehicle for all transfers',
            '3/4-Star verified hotel stays with daily breakfast and dinner',
            '24/7 dedicated trip coordinator assistance',
          ],
          inclusions: [
            '4 Nights accommodation in 3/4-Star hotels',
            'Daily Breakfast & Dinner',
            'Private vehicle with experienced chauffeur',
            'All state tolls, taxes and parking fees',
          ],
          exclusions: ['Flight / Train tickets', 'Personal expenses & adventure activity fees'],
          hotelType: '3/4-Star Deluxe Hotels',
          pickupDrop: `Nearest Airport / Railway Station to ${formattedName}`,
          itinerary: [
            {
              day: 1,
              title: `Arrival in ${formattedName} & Welcome`,
              description: `Arrive and meet our representative. Transfer to your hotel, check in, and enjoy a relaxing evening.`,
              activities: ['Arrival Transfer', 'Hotel Check-in', 'Evening Stroll'],
              meals: 'Dinner',
              stay: `Deluxe Hotel, ${formattedName}`,
            },
            {
              day: 2,
              title: 'Iconic City Sightseeing & Landmarks',
              description: `Full day exploration of the top historic and natural landmarks of ${formattedName}.`,
              activities: ['Landmark Tours', 'Local Cuisine Tasting'],
            ...
            },
            {
              ...
              description: `Scenic excursion to nearby scenic spots and cultural markets.`,
              activities: ['Scenic Drive', 'Photography', 'Souvenir Shopping'],
              meals: 'Breakfast & Dinner',
              stay: `Deluxe Hotel, ${formattedName}`,
            },
            {
              day: 4,
              title: 'Cultural Heritage & Leisure Day',
              description: `Visit local handicraft centers and enjoy leisure time.`,
              activities: ['Handicraft Centers', 'Sunset Point'],
              meals: 'Breakfast & Dinner',
              stay: `Deluxe Hotel, ${formattedName}`,
            },
            {
              ...
              description: `Breakfast, check out, and transfer for your journey home.`,
              activities: ['Hotel Check-out', 'Departure Drop'],
              meals: 'Breakfast',
              stay: 'End of Tour',
            },
          ],
        ...
        <div className="relative z-10 text-center px-4 sm:px-6 max-w-4xl mx-auto flex flex-col items-center my-auto">
          ...
          <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md border border-white/20 px-4 py-1.5 rounded-full text-white text-[12.5px] font-medium mb-4 shadow">
            ...
            <span className="text-[#f26c22] font-semibold flex items-center gap-1">
              <FaMapMarkerAlt className="text-xs" /> {dest.name}
            </span>
          </div>
          ...
          <div className="flex items-center justify-center gap-3 sm:gap-6 flex-wrap text-white text-xs sm:text-sm font-semibold">
            <span className="bg-white/15 backdrop-blur-md px-3.5 py-1.5 rounded-full border border-white/20 flex items-center gap-1.5">
              <FaCheckCircle className="text-[#f26c22]" /> {dest.packages.length} Handcrafted {dest.packages.length === 1 ? 'Package' : 'Packages'}
            </span>
            <span className="bg-white/15 backdrop-blur-md px-3.5 py-1.5 rounded-full border border-white/20 flex items-center gap-1.5">
              <FaStar className="text-amber-400" /> Best Time: {dest.bestTime.split('(')[0]}
            </span>
            <span className="bg-white/15 backdrop-blur-md px-3.5 py-1.5 rounded-full border border-white/20 flex items-center gap-1.5">
              <FaCar className="text-emerald-400" /> Private Sanitized Cab Included
            </span>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-8 lg:px-16 max-w-[1400px] mx-auto w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6 border-b border-gray-200/80 pb-8">
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
            {['All', 'Popular', 'Adventure', 'Family', 'Honeymoon'].map((cat) => (
              <button
                key={cat}
            ...
            >
              <div>
                ...
                <div className="p-6">
                  ...
                  <Link href={`/national/${dest?.slug}/${pkg.slug}`}>
              ...
              <div className="p-6 pt-0 border-t border-gray-100 flex items-center justify-between mt-auto">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Link
                    href={`/national/${dest?.slug}/${pkg.slug}`}
      ...
      <section className="py-16 px-4 sm:px-8 text-center bg-white border-t border-gray-100">
        <div className="max-w-4xl mx-auto bg-gradient-to-br from-[#111827] to-[#1e293b] text-white rounded-3xl p-8 sm:p-12 shadow-2xl relative overflow-hidden">
          <h2 className="text-[28px] sm:text-[36px] font-black mb-3 leading-tight">
            Need a Customized {dest.name} Itinerary?
          </h2>
          <p className="text-gray-300 text-sm sm:text-base max-w-xl mx-auto mb-8 font-light">
            Share your travel dates and passenger details. We will build a custom trip plan with guaranteed lowest rates and instant confirmation.
          </p>
          ...
          <div className="flex items-center justify-center gap-4 flex-wrap">
            ...
            <a
              href={`https://wa.me/918091660060?text=Hello%20Zoyo%20Trip,%20I%20want%20to%20plan%20a%20trip%20to%20${encodeURIComponent(dest.name)}`}
              target="_blank"
              rel="noreferrer"
