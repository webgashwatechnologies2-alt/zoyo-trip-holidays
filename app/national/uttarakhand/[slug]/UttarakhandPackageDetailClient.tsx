...
import type { TourPackage } from '@/data/uttarakhandPackages';
...
export default function UttarakhandPackageDetailClient({
  ...
  const pkg: TourPackage | undefined = uttarakhandPackages.find((p) => p.slug === params.slug);
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-7 border border-gray-100 shadow-[0_10px_35px_rgba(0,0,0,0.08)]">
              ...
              {formSubmitted ? (
                <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-6 text-center text-emerald-900">
                  <div className="w-12 h-12 rounded-full bg-emerald-500 text-white flex items-center justify-center text-2xl mx-auto mb-3">✓</div>
                  <h4 className="font-bold text-base mb-1">Quote Request Sent!</h4>
                  <p className="text-xs text-emerald-700 leading-relaxed mb-4">
                    Our Uttarakhand travel consultant will contact you via WhatsApp &amp; Call.
                  </p>
                  <button onClick={() => setFormSubmitted(false)} className="text-xs font-bold text-[#111827] underline cursor-pointer">
                ...
                <form onSubmit={handleBookingSubmit} className="space-y-3.5">
                  ...
                  <button type="submit" className="w-full bg-[#f26c22] hover:bg-[#d95d1a] text-white py-3.5 rounded-xl font-bold text-xs transition shadow-md flex items-center justify-center gap-2 cursor-pointer">
              ...
              <div className="mt-3 text-center">
                <a href="tel:+918091660060" className="text-xs font-semibold text-gray-600 hover:text-[#1E6AD4] flex items-center justify-center gap-1.5">
              ...
              <div className="mt-5 pt-4 border-t border-gray-100 space-y-2 text-[11.5px] text-gray-500">
                <div className="flex items-center gap-2"><FaShieldAlt className="text-[#10b981]" /><span>100% Safe Payments (Credit Card / UPI / NetBanking)</span></div>
                <div className="flex items-center gap-2"><FaAward className="text-[#f26c22]" /><span>Government Registered Tour Operator</span></div>
                <div className="flex items-center gap-2"><FaCheckCircle className="text-[#1E6AD4]" /><span>No Hidden Surcharges &amp; Transparent Billing</span></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-20 pt-12 border-t border-gray-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {otherPackages.map((item) => (
              <div key={item.id} className="group bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.06)] hover:shadow-[0_16px_40px_rgba(0,0,0,0.12)] hover:-translate-y-1.5 transition-all duration-300 flex flex-col justify-between">
                <div>
                  <div className="relative h-48 overflow-hidden bg-gray-100">
                    <img src={item.heroImage} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
                    <div className="absolute top-3 left-3 bg-[#f26c22] text-white text-[10px] font-black px-2.5 py-0.5 rounded-full uppercase">{item.category}</div>
                    <div className="absolute bottom-3 left-3 text-white text-xs font-semibold">{item.duration}</div>
                  </div>
                  <div className="p-5">
                    <div className="text-xs text-[#1E6AD4] font-semibold mb-1 truncate">{item.location}</div>
                    <h3 className="font-bold text-gray-900 group-hover:text-[#f26c22] transition-colors text-base line-clamp-1 mb-2">{item.title}</h3>
                ...
                <div className="p-5 pt-0 border-t border-gray-100 flex items-center justify-between mt-4">
                  ...
                  <Link href={`/national/uttarakhand/${item.slug}`} className="bg-[#111827] hover:bg-[#f26c22] text-white px-4 py-2 rounded-xl font-bold text-xs transition-colors flex items-center gap-1">
                    <span>View Tour</span>
                    <BsArrowRight />
                  </Link>
                </div>
